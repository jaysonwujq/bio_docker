...
RUN mkdir /ref/ && cd /ref/ && axel -n20 http://www.openbioinformatics.org/annovar/download/0wgxR2rIVP/annovar.latest.tar.gz && \
...
RUN cd /ref/annovar/humandb/ && axel -n20 http://www.openbioinformatics.org/annovar/download/hg19_gnomad211_exome.txt.gz
RUN cd /ref/annovar/humandb/ && axel -n20 http://www.openbioinformatics.org/annovar/download/hg19_gnomad211_exome.txt.idx.gz
RUN rm -rf /var/lib/apt/lists/* /tmp/* /var/tmp/* /var/log/messages && cd /ref/annovar/humandb/ && gunzip *gz && unzip hg19_1000g2015aug.zip
